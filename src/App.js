...
function persist(d) { try { localStorage.setItem(SKEY,JSON.stringify(d)); } catch {} }

const DEFAULT_USERS = [
  {id:"u1",username:"admin",pinHash:hashPIN("1234"),role:"admin"},
  {id:"u2",username:"staff1",pinHash:hashPIN("0000"),role:"staff"},
];

// products: [{id, sku, modelCode, storageCode, colourCode, gradeCode, specCode, categoryCode(optional), purchasePrice, salePrice}]
const INIT = {
  users: DEFAULT_USERS,
  shelves: [{id:"shelf1",name:"Shelf A"},{id:"shelf2",name:"Shelf B"}],
  products: [
    {id:"p1",sku:"IP14P128BKU-A",modelCode:"IP14P",storageCode:"128",colourCode:"BK",specCode:"U",gradeCode:"A",categoryCode:"",purchasePrice:"480",salePrice:"620"},
    {id:"p2",sku:"SGS23128BKU-B",modelCode:"SGS23",storageCode:"128",colourCode:"BK",specCode:"U",gradeCode:"B",categoryCode:"",purchasePrice:"280",salePrice:"390"},
    {id:"p3",sku:"IP15PM256NTG-A",modelCode:"IP15PM",storageCode:"256",colourCode:"NT",specCode:"G",gradeCode:"A",categoryCode:"",purchasePrice:"700",salePrice:"900"},
  ],
  trays: [
    {id:"tray1",productId:"p1",shelfId:"shelf1"},
    {id:"tray2",productId:"p2",shelfId:"shelf1"},
    {id:"tray3",productId:"p3",shelfId:"shelf2"},
  ],
  devices: [
    {id:"d1",imei:"352099001761481",trayId:"tray1",status:"In Stock",addedAt:nowISO(),addedBy:"admin",history:[]},
    {id:"d2",imei:"352099001761482",trayId:"tray1",status:"In Stock",addedAt:nowISO(),addedBy:"admin",history:[]},
    {id:"d3",imei:"358697101234567",trayId:"tray2",status:"In Stock",addedAt:nowISO(),addedBy:"admin",history:[]},
    {id:"d4",imei:"867530021234568",trayId:"tray3",status:"In Stock",addedAt:nowISO(),addedBy:"admin",history:[]},
  ],
  customModelCodes:[],customColourCodes:[],customStorageCodes:[],
  customSpecCodes:[],customGradeCodes:[],customCategories:[],
  globalHistory:[],
};
...
function Barcode({value,width=280,height=60}) {
  const ref=useRef();
  useEffect(()=>{
    // Only use printable ASCII (32–126)
    const safe=value.replace(/[^\x20-\x7E]/g,"");
    if(!safe) return;
    const bars=C128(safe),canvas=ref.current;
    if(!canvas) return;
...
function LoginScreen({users,onLogin}) {
  ...
  const submit=()=>{ const u=users.find(u=>u.username.toLowerCase()===username.trim().toLowerCase()&&u.pinHash===hashPIN(pin)); if(!u){setErr("Invalid username or PIN.");setPin("");return;} onLogin(u); };
  return (
    <div style={{minHeight:"100vh",background:"#0f172a",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#fff",borderRadius:20,padding:40,width:360,boxShadow:"0 20px 60px rgba(0,0,0,0.4)"}}>
        ...
        <button onClick={submit} style={{width:"100%",background:"#3b82f6",border:"none",borderRadius:10,padding:"13px",cursor:"pointer",fontWeight:800,color:"#fff",fontSize:15}}>Sign In</button>
        <div style={{marginTop:18,background:"#f8fafc",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#64748b"}}><b>Demo:</b> admin / 1234 &nbsp;|&nbsp; staff1 / 0000</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  // Merged catalogs
  const allModelCodes=useMemo(()=>[...MODEL_CODES,...(data.customModelCodes||[])]   ,[data.customModelCodes]);
  const allColourCodes=useMemo(()=>[...COLOUR_CODES,...(data.customColourCodes||[])] ,[data.customColourCodes]);
  const allStorageCodes=useMemo(()=>[...STORAGE_CODES,...(data.customStorageCodes||[])]  ,[data.customStorageCodes]);
  const allSpecCodes=useMemo(()=>[...SPEC_CODES,...(data.customSpecCodes||[])]       ,[data.customSpecCodes]);
  const allGradeCodes=useMemo(()=>[...GRADE_CODES,...(data.customGradeCodes||[])]    ,[data.customGradeCodes]);
  const allCategories=useMemo(()=>[...(data.customCategories||[])]                   ,[data.customCategories]);

  const trayProduct=useMemo(()=>{ const m={}; data.trays.forEach(t=>{ m[t.id]=data.products.find(p=>p.id===t.productId); }); return m; },[data.trays,data.products]);
  const trayDevices=useMemo(()=>{ const m={}; data.trays.forEach(t=>{ m[t.id]=data.devices.filter(d=>d.trayId===t.id); }); return m; },[data.devices,data.trays]);
  const trayStock=useMemo(()=>{ const m={}; data.trays.forEach(t=>{ m[t.id]=data.devices.filter(d=>d.trayId===t.id&&d.status==="In Stock").length; }); return m; },[data.devices,data.trays]);
  const shelfTrays=useMemo(()=>{ const m={}; data.shelves.forEach(s=>{ m[s.id]=data.trays.filter(t=>t.shelfId===s.id); }); return m; },[data.trays,data.shelves]);
  const imeiMap=useMemo(()=>{ const m={}; data.devices.forEach(d=>{ m[d.imei]=d; }); return m; },[data.devices]);
  const skuSet=useMemo(()=>new Set(data.products.map(p=>p.sku)),[data.products]);

  const logAction=(deviceId,action,detail,by)=>{
    const entry={id:genId(),deviceId,action,detail,by:by||session?.user.username||"?",at:nowISO()};
    setData(prev=>({...prev,devices:prev.devices.map(d=>d.id===deviceId?{...d,history:[entry,...(d.history||[])]}:d),globalHistory:[entry,...prev.globalHistory]}));
  };

  if(!session) return <LoginScreen users={data.users} onLogin={u=>{ setSession({user:u,loginAt:nowISO()}); setTab("scanner"); }}/>;
    ...
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"#f1f5f9",minHeight:"100vh"}}>
      ...
      <div style={{background:"#0f172a",color:"#fff",padding:"12px 24px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <span style={{fontSize:13,color:"#94a3b8"}}>📦 {data.devices.filter(d=>d.status==="In Stock").length} in stock</span>
          <RoleBadge role={session.user.role}/>
          <span style={{fontSize:13,color:"#60a5fa",fontWeight:600}}>{session.user.username}</span>
      ...
      <div style={{padding:"20px 24px"}}>
        {tab==="scanner"  &&<ScannerTab data={data} setData={setData} trayStock={trayStock} trayDevices={trayDevices} trayProduct={trayProduct} imeiMap={imeiMap} showToast={showToast} logAction={logAction} session={session} catalog={catalog}/>}
        {tab==="shelves"  &&isAdmin&&<ShelvesTab data={data} setData={setData} shelfTrays={shelfTrays} trayStock={trayStock} trayProduct={trayProduct} showToast={showToast}/>}
        {tab==="trays"    &&isAdmin&&<TraysTab data={data} setData={setData} trayStock={trayStock} trayDevices={trayDevices} trayProduct={trayProduct} shelfTrays={shelfTrays} showToast={showToast} catalog={catalog}/>}
        {tab==="devices"  &&<DevicesTab data={data} trayProduct={trayProduct} catalog={catalog}/>}
        {tab==="history"  &&<HistoryTab data={data}/>}
        {tab==="dashboard"&&isAdmin&&<DashboardTab data={data} trayStock={trayStock} trayProduct={trayProduct} catalog={catalog}/>}
        {tab==="admin"    &&isAdmin&&<AdminTab data={data} setData={setData} showToast={showToast} session={session} catalog={catalog} skuSet={skuSet}/>}
      </div>
    </div>
  );
}
...
function productDesc(p, catalog) {
  ...
  const parts=[labelFor(allModelCodes,p.modelCode),labelFor(allStorageCodes,p.storageCode),labelFor(allColourCodes,p.colourCode),labelFor(allSpecCodes,p.specCode),labelFor(allGradeCodes,p.gradeCode)];
  if(p.categoryCode) parts.push(labelFor(allCategories,p.categoryCode));
  return parts.join(" · ");
}

// ─── SCANNER ──────────────────────────────────────────────────────────────────
function ScannerTab({data,setData,trayStock,trayDevices,trayProduct,imeiMap,showToast,logAction,session,catalog}) {
  const beep=useBeep();
  ...
  const currentTrayStock=useMemo(()=>tray?data.devices.filter(d=>d.trayId===tray.id&&d.status==="In Stock").length:0,[data.devices,tray]);
  ...
  const handleScan=()=>{
    ...
    // Always re-focus after processing — critical for USB scanner flow
    setTimeout(()=>inputRef.current?.focus(),30);
    if(phase==="idle"||phase==="mode_select") {
      const found=data.trays.find(t=>{ const p=trayProduct[t.id]; return p&&p.sku===v; });
    ...
    if(phase==="intake") {
      ...
      const newDev={id:genId(),imei:v,trayId:tray.id,status:"In Stock",addedAt:nowISO(),addedBy:session.user.username,history:[{id:genId(),action:"Added",detail:`Added to tray ${product?.sku}`,by:session.user.username,at:nowISO()}]};
      setData(prev=>({...prev,devices:[...prev.devices,newDev],globalHistory:[{id:genId(),deviceId:newDev.id,action:"Added",detail:`Added to tray ${product?.sku}`,by:session.user.username,at:nowISO()},...prev.globalHistory]}));
    ...
    if(phase==="dispatch") {
      ...
      if(dev.trayId!==tray.id){ const wp=trayProduct[dev.trayId]; beep.error(); fb("error",`Belongs to ${wp?.sku||"another tray"}.`); setScanLog(p=>[{imei:v,ok:false,msg:`Wrong tray: ${wp?.sku||"?"}`},...p]); return; }
      ...
      if(dispatchQueue.includes(dev.id)){ beep.error(); fb("error","Already queued."); return; }
      setDispatchQueue(p=>[...p,dev.id]); beep.success();
      setScanLog(p=>[{imei:v,ok:true,msg:"Queued"},...p]);
  ...
  const confirmDispatch=()=>{
    ...
    setData(prev=>({
      ...prev,
      devices:prev.devices.map(d=>dispatchQueue.includes(d.id)?{...d,status:"Sold",history:[{id:genId(),action:"Dispatched",detail:reason,by:session.user.username,at:now},...(d.history||[])]}:d),
      globalHistory:[...dispatchQueue.map(id=>({id:genId(),deviceId:id,action:"Dispatched",detail:`Sold — ${reason}`,by:session.user.username,at:now})),...prev.globalHistory],
    }));
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 290px",gap:16,alignItems:"start"}}>
        ...
        <div style={{background:"#0f172a",borderRadius:14,padding:20,marginBottom:14}}>
            ...
            <div>
              <div style={{display:"flex",gap:10}}>
                <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value.toUpperCase())} onKeyDown={e=>e.key==="Enter"&&handleScan()}
                  placeholder={phase==="intake"?"Scan IMEI to add...":"Scan IMEI to dispatch..."}
                  style={{flex:1,padding:"13px 16px",borderRadius:10,border:`2px solid ${modeColor[phase]}`,fontSize:15,fontFamily:"monospace",background:"#1e293b",color:"#f1f5f9",outline:"none",letterSpacing:"0.05em"}}/>
                <button onClick={handleScan} style={{background:modeColor[phase],border:"none",borderRadius:10,padding:"0 22px",color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer"}}>GO</button>
              </div>
            ...
            <div>
              <div style={{background:"#1e293b",borderRadius:10,padding:16,marginBottom:14}}>
                ...
                <div style={{maxHeight:160,overflowY:"auto"}}>
                  {dispatchQueue.map(id=>{ const dev=data.devices.find(d=>d.id===id); return <div key={id} style={{fontFamily:"monospace",fontSize:13,color:"#e2e8f0",padding:"4px 0",borderBottom:"1px solid #334155"}}>{dev?.imei}</div>; })}
                </div>
              </div>
          ...
          {phase==="idle"&&(
            <div style={{display:"flex",gap:10,marginTop:4}}>
              <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value.toUpperCase())} onKeyDown={e=>e.key==="Enter"&&handleScan()}
                placeholder="Scan tray SKU to begin..."
                style={{flex:1,padding:"13px 16px",borderRadius:10,border:"2px solid #334155",fontSize:15,fontFamily:"monospace",background:"#1e293b",color:"#f1f5f9",outline:"none",letterSpacing:"0.05em"}}/>
              <button onClick={handleScan} style={{background:"#3b82f6",border:"none",borderRadius:10,padding:"0 22px",color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer"}}>GO</button>
            </div>
          )}
        </div>
        {scanLog.length>0&&(
            ...
            <div style={{maxHeight:220,overflowY:"auto"}}>
              {scanLog.map((s,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 10px",borderRadius:6,background:s.ok?"#f0fdf4":"#fef2f2",marginBottom:4}}>
                  <span>{s.ok?"✅":"❌"}</span>
                  <span style={{fontFamily:"monospace",fontSize:13,flex:1}}>{s.imei}</span>
                  <span style={{fontSize:12,color:s.ok?"#065f46":"#dc2626",fontWeight:600}}>{s.msg}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {tray&&(
          <div style={{background:"#fff",borderRadius:12,padding:18,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:3,marginTop:12}}>
              {Array.from({length:MAX_TRAY}).map((_,i)=>(
                <div key={i} style={{width:"100%",paddingTop:"100%",borderRadius:3,background:i<currentTrayStock?"#10b981":"#e2e8f0"}}/>
              ))}
            </div>
          </div>
        )}
        <div style={{background:"#fff",borderRadius:12,padding:18,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
          ...
          {[["In Stock",data.devices.filter(d=>d.status==="In Stock").length,"#10b981"],["Total Devices",data.devices.length,"#3b82f6"],["Products",data.products.length,"#8b5cf6"],["Trays",data.trays.length,"#f59e0b"]].map(([l,v,c])=>(
...
// ─── SHELVES ──────────────────────────────────────────────────────────────────
function ShelvesTab({data,setData,shelfTrays,trayStock,trayProduct,showToast}) {
  const [newName,setNewName]=useState("");
  const [editShelf,setEditShelf]=useState(null);
  const add=()=>{ if(!newName.trim()) return showToast("Enter name.","error"); if(data.shelves.find(s=>s.name.toLowerCase()===newName.trim().toLowerCase())) return showToast("Name exists.","error"); setData(p=>({...p,shelves:[...p.shelves,{id:genId(),name:newName.trim()}]})); setNewName(""); showToast("Shelf added."); };
  const del=id=>{ if((shelfTrays[id]||[]).length>0) return showToast("Remove trays first.","error"); setData(p=>({...p,shelves:p.shelves.filter(s=>s.id!==id)})); showToast("Shelf removed."); };
  const saveEdit=()=>{ setData(p=>({...p,shelves:p.shelves.map(s=>s.id===editShelf.id?{...s,name:editShelf.name.trim()}:s)})); setEditShelf(null); showToast("Renamed."); };
  return (
    <div>
...
// ─── TRAYS ────────────────────────────────────────────────────────────────────
function TraysTab({data,setData,trayStock,trayDevices,trayProduct,shelfTrays,showToast,catalog}) {
  const {allModelCodes,allColourCodes,allStorageCodes,allSpecCodes,allGradeCodes,allCategories}=catalog;
  ...
  useEffect(()=>{
    ...
    const found=data.products.find(p=>p.modelCode===form.modelCode&&p.storageCode===form.storageCode&&p.colourCode===form.colourCode&&p.specCode===form.specCode&&p.gradeCode===form.gradeCode&&(p.categoryCode||"")===form.categoryCode);
    setMatchedProduct(found||null); setNoMatch(!found);
  },[form,data.products]);

  const addTray=()=>{
    if(!matchedProduct) return showToast("No matching product found. Create it in the Product Catalog first.","error");
    ...
    const already=data.trays.find(t=>t.productId===matchedProduct.id&&t.shelfId===form.shelfId);
    if(already) return showToast(`A tray for ${matchedProduct.sku} already exists on this shelf.`,"error");
    setData(prev=>({...prev,trays:[...prev.trays,{id:genId(),productId:matchedProduct.id,shelfId:form.shelfId}]}));
    showToast(`Tray for ${matchedProduct.sku} created.`); setShowAdd(false); setForm({modelCode:"",storageCode:"",colourCode:"",specCode:"",gradeCode:"",categoryCode:"",shelfId:""});
  };

  const [clearTray,setClearTray]=useState(null);
  const [clearReason,setClearReason]=useState("");

  const confirmClearTray=()=>{
    if(!clearReason) return showToast("Select a reason.","error");
    const now=nowISO();
    const inStockIds=data.devices.filter(d=>d.trayId===clearTray.id&&d.status==="In Stock").map(d=>d.id);
    const p=trayProduct[clearTray.id];
    setData(prev=>({
      ...prev,
      devices:prev.devices.map(d=>inStockIds.includes(d.id)?{...d,status:"Sold",history:[{id:genId(),action:"Tray Clear",detail:`Audit clear — ${clearReason}`,by:"admin",at:now},...(d.history||[])]}:d),
      globalHistory:[...inStockIds.map(id=>({id:genId(),deviceId:id,action:"Tray Cleared",detail:`Audit — ${clearReason} — ${p?.sku||""}`,by:"admin",at:now})),...prev.globalHistory],
    }));
    showToast(`Tray ${p?.sku} cleared — ${inStockIds.length} device(s) logged.`);
    setClearTray(null); setClearReason("");
  };

  const deleteTray=id=>{ if((trayDevices[id]||[]).some(d=>d.status==="In Stock")) return showToast("Tray has stock. Clear or dispatch all first.","error"); setData(prev=>({...prev,trays:prev.trays.filter(t=>t.id!==id)})); showToast("Tray removed."); };
  const filtered=filterShelf==="All"?data.trays:data.trays.filter(t=>t.shelfId===filterShelf);
    ...
    <div>
      <div style={{display:"flex",gap:10,marginBottom:16,alignItems:"center"}}>
        <select value={filterShelf} onChange={e=>setFilterShelf(e.target.value)} style={{padding:"9px 12px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:14}}>
          ...
          {data.shelves.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}
        </select>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
        {filtered.map(tray=>{
          const p=trayProduct[tray.id]; const shelf=data.shelves.find(s=>s.id===tray.shelfId); const stock=trayStock[tray.id]||0;
          return (
            <div key={tray.id} style={{background:"#fff",borderRadius:12,padding:18,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                ...
                <div style={{display:"flex",gap:5}}>
                  ...
                  {(trayStock[tray.id]||0)>0&&<button onClick={()=>{setClearTray(tray);setClearReason("");}} style={{background:"#fff7ed",border:"none",borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:12,fontWeight:600,color:"#ea580c"}}>🗑 Clear</button>}
              ...
              {p&&<div style={{display:"flex",justifyContent:"space-between",marginTop:10,fontSize:12,color:"#64748b"}}>
                <span>Cost: {fmtGBP(p.purchasePrice)}</span><span>Sale: {fmtGBP(p.salePrice)}</span>
                <span style={{fontWeight:700,color:"#10b981"}}>Val: {fmtGBP(parseFloat(p.salePrice||0)*stock)}</span>
              </div>}
            </div>
          );
        })}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
              {[{label:"Model *",key:"modelCode",opts:allModelCodes},{label:"Storage *",key:"storageCode",opts:allStorageCodes},{label:"Colour *",key:"colourCode",opts:allColourCodes},{label:"Spec *",key:"specCode",opts:allSpecCodes},{label:"Grade *",key:"gradeCode",opts:allGradeCodes}].map(f=>(
                <div key={f.key}>
                  ...
                  <select value={form[f.key]} onChange={e=>sel(f.key,e.target.value)} style={selStyle}>
                    <option value="">— Select —</option>
                    {f.opts.map(o=><option key={o.code} value={o.code}>{o.label} ({o.code})</option>)}
                  </select>
                </div>
              ))}
              <div>
                <label style={{fontSize:12,fontWeight:600,color:"#64748b",display:"block",marginBottom:4}}>Category <span style={{fontWeight:400,color:"#94a3b8"}}>(optional)</span></label>
                <select value={form.categoryCode} onChange={e=>sel("categoryCode",e.target.value)} style={selStyle}>
                  <option value="">— None —</option>
                  {allCategories.map(c=><option key={c.code} value={c.code}>{c.label} ({c.code})</option>)}
                </select>
              </div>
            </div>

            {/* Product match feedback */}
            {matchedProduct&&(
              <div style={{background:"#f0fdf4",border:"1px solid #6ee7b7",borderRadius:10,padding:"12px 16px",marginBottom:16}}>
                <div style={{fontSize:12,fontWeight:700,color:"#065f46",marginBottom:4}}>✅ Product found</div>
                <div style={{fontFamily:"monospace",fontWeight:800,fontSize:16,color:"#0f172a"}}>{matchedProduct.sku}</div>
                <div style={{fontSize:13,color:"#475569",marginTop:2}}>{productDesc(matchedProduct,catalog)}</div>
              </div>
            )}
            {noMatch&&(
              <div style={{background:"#fef2f2",border:"1px solid #fca5a5",borderRadius:10,padding:"12px 16px",marginBottom:16}}>
                <div style={{fontSize:13,fontWeight:700,color:"#dc2626",marginBottom:2}}>⚠️ No product found for this combination</div>
                <div style={{fontSize:12,color:"#64748b"}}>Go to <b>Admin → Product Catalog → Products</b> and create this product first.</div>
              </div>
            )}

            <div style={{marginBottom:16}}>
              <label style={{fontSize:12,fontWeight:600,color:"#64748b",display:"block",marginBottom:4}}>Shelf *</label>
              <select value={form.shelfId} onChange={e=>sel("shelfId",e.target.value)} style={selStyle}>
      ...
      {printTray&&<PrintModal tray={printTray} data={data} trayProduct={trayProduct} catalog={catalog} onClose={()=>setPrintTray(null)}/>}
      ...
      {clearTray&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"#fff",borderRadius:16,padding:28,width:420,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
            <div style={{fontWeight:800,fontSize:17,marginBottom:4,color:"#0f172a"}}>🗑 Clear Tray</div>
            <div style={{fontSize:13,color:"#64748b",marginBottom:18}}>
              <span style={{fontFamily:"monospace",fontWeight:700,color:"#0f172a"}}>{trayProduct[clearTray.id]?.sku}</span> — {trayStock[clearTray.id]||0} device(s) in stock will be marked as Sold and logged.
            </div>
            <div style={{background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:10,padding:"12px 14px",marginBottom:18,fontSize:13,color:"#92400e",fontWeight:600}}>
              ⚠️ This action cannot be undone. All in-stock IMEIs will be cleared.
            </div>
            <div style={{fontSize:12,fontWeight:700,color:"#64748b",marginBottom:8}}>SELECT AUDIT REASON</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:20}}>
              {["Stock Count Audit","Write-Off","Batch Sale","Lost Stock","Other"].map(r=>(
                <button key={r} onClick={()=>setClearReason(r)} style={{background:clearReason===r?"#ea580c":"#f1f5f9",border:`1px solid ${clearReason===r?"#ea580c":"#e2e8f0"}`,borderRadius:8,padding:"7px 14px",cursor:"pointer",fontWeight:600,fontSize:13,color:clearReason===r?"#fff":"#374151"}}>{r}</button>
              ))}
            </div>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
...
// ─── PRINT MODAL ──────────────────────────────────────────────────────────────
function PrintModal({tray,data,trayProduct,catalog,onClose}) {
  const p=trayProduct[tray.id];
  const shelf=data.shelves.find(s=>s.id===tray.shelfId);
...
// ─── DEVICES ──────────────────────────────────────────────────────────────────
function DevicesTab({data,trayProduct,catalog}) {
  const [search,setSearch]=useState("");
  const [filterStatus,setFilterStatus]=useState("All");
  const [filterTray,setFilterTray]=useState("All");
  const filtered=useMemo(()=>{
    let res=data.devices;
    const q=search.toLowerCase();
    if(q) res=res.filter(d=>d.imei.includes(q)||(trayProduct[d.trayId]?.sku||"").toLowerCase().includes(q));
    if(filterStatus!=="All") res=res.filter(d=>d.status===filterStatus);
    if(filterTray!=="All") res=res.filter(d=>d.trayId===filterTray);
    return res;
  },[data.devices,trayProduct,search,filterStatus,filterTray]);
  return (
    <div>
      <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
        ...
        <select value={filterTray} onChange={e=>setFilterTray(e.target.value)} style={{padding:"9px 12px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:14}}>
          <option value="All">All Trays</option>{data.trays.map(t=>{ const p=trayProduct[t.id]; return <option key={t.id} value={t.id}>{p?.sku||t.id}</option>; })}
        </select>
      ...
      <div style={{background:"#fff",borderRadius:12,boxShadow:"0 1px 4px rgba(0,0,0,0.07)",overflow:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",minWidth:650}}>
          ...
          <tbody>
            ...
            {filtered.map((dev,i)=>{ const p=trayProduct[dev.trayId]; const shelf=data.shelves.find(s=>s.id===data.trays.find(t=>t.id===dev.trayId)?.shelfId); return (
              <tr key={dev.id} style={{background:i%2===0?"#fff":"#fafafa"}}>
                ...
                <td style={{padding:"9px 12px",borderBottom:"1px solid #f1f5f9"}}>{p?<GradeBadge code={p.gradeCode} allGrades={catalog.allGradeCodes}/>:"—"}</td>
                <td style={{padding:"9px 12px",borderBottom:"1px solid #f1f5f9"}}><StatusBadge status={dev.status}/></td>
                <td style={{padding:"9px 12px",fontSize:12,color:"#3b82f6",fontWeight:600,borderBottom:"1px solid #f1f5f9"}}>{dev.addedBy||"—"}</td>
                <td style={{padding:"9px 12px",fontSize:12,color:"#94a3b8",borderBottom:"1px solid #f1f5f9",whiteSpace:"nowrap"}}>{fmtDate(dev.addedAt)}</td>
              </tr>
            );})}
          </tbody>
        </table>
...
function HistoryTab({data}) {
  const [search,setSearch]=useState("");
  const filtered=useMemo(()=>{ const q=search.toLowerCase(); if(!q) return data.globalHistory; return data.globalHistory.filter(h=>h.action?.toLowerCase().includes(q)||h.detail?.toLowerCase().includes(q)||h.by?.toLowerCase().includes(q)||data.devices.find(d=>d.id===h.deviceId)?.imei.includes(q)); },[data,search]);
  return (
    <div>
      ...
      <div style={{background:"#fff",borderRadius:12,boxShadow:"0 1px 4px rgba(0,0,0,0.07)",overflow:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>
            ...
            {filtered.map((h,i)=>{ const dev=data.devices.find(d=>d.id===h.deviceId); return (
              <tr key={h.id} style={{background:i%2===0?"#fff":"#fafafa"}}>
                ...
                <td style={{padding:"9px 12px",fontSize:13,fontWeight:600,color:"#3b82f6",borderBottom:"1px solid #f1f5f9"}}>{h.by||"—"}</td>
                <td style={{padding:"9px 12px",fontFamily:"monospace",fontSize:13,borderBottom:"1px solid #f1f5f9"}}>{dev?.imei||"—"}</td>
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function DashboardTab({data,trayStock,trayProduct,catalog}) {
  const stats=useMemo(()=>{
    const inStock=data.devices.filter(d=>d.status==="In Stock");
    const totalCost=inStock.reduce((s,d)=>{ const p=trayProduct[d.trayId]; return s+(parseFloat(p?.purchasePrice)||0); },0);
    const totalSale=inStock.reduce((s,d)=>{ const p=trayProduct[d.trayId]; return s+(parseFloat(p?.salePrice)||0); },0);
    const byGrade=catalog.allGradeCodes.map(g=>({grade:g.code,label:g.label,count:inStock.filter(d=>trayProduct[d.trayId]?.gradeCode===g.code).length}));
    const bySpec=catalog.allSpecCodes.map(s=>({spec:s.code,label:s.label,count:inStock.filter(d=>trayProduct[d.trayId]?.specCode===s.code).length}));
    return {inStock:inStock.length,sold:data.devices.filter(d=>d.status==="Sold").length,repair:data.devices.filter(d=>d.status==="In Repair").length,transit:data.devices.filter(d=>d.status==="In Transit").length,totalCost,totalSale,profit:totalSale-totalCost,byGrade,bySpec,fullTrays:data.trays.filter(t=>(trayStock[t.id]||0)>=MAX_TRAY),emptyTrays:data.trays.filter(t=>(trayStock[t.id]||0)===0)};
  },[data,trayStock,trayProduct,catalog]);
  return (
    <div>
...
// ─── Stable CatalogSection (defined outside component to prevent re-creation) ──
...
function AdminTab({data,setData,showToast,session,catalog,skuSet}) {
  const {allModelCodes,allColourCodes,allStorageCodes,allSpecCodes,allGradeCodes,allCategories}=catalog;
  ...
  const addUser=()=>{ if(!newUser.username.trim()) return showToast("Username required.","error"); if(!/^\d{4,6}$/.test(newUser.pin)) return showToast("PIN must be 4-6 digits.","error"); if(data.users.find(u=>u.username.toLowerCase()===newUser.username.trim().toLowerCase())) return showToast("Username exists.","error"); setData(prev=>({...prev,users:[...prev.users,{id:genId(),username:newUser.username.trim(),pinHash:hashPIN(newUser.pin),role:newUser.role}]})); showToast(`User "${newUser.username}" created.`); setShowAddUser(false); setNewUser({username:"",pin:"",role:"staff"}); };
  const deleteUser=id=>{ if(id===session.user.id) return showToast("Cannot delete yourself.","error"); setData(prev=>({...prev,users:prev.users.filter(u=>u.id!==id)})); showToast("User removed."); };
  const savePIN=()=>{ if(!/^\d{4,6}$/.test(newPIN)) return showToast("PIN must be 4-6 digits.","error"); setData(prev=>({...prev,users:prev.users.map(u=>u.id===changePIN?{...u,pinHash:hashPIN(newPIN)}:u)})); showToast("PIN updated."); setChangePIN(null); setNewPIN(""); };
  ...
  const addProduct=()=>{
    ...
    const dupCombo=data.products.find(p=>p.modelCode===modelCode&&p.storageCode===storageCode&&p.colourCode===colourCode&&p.specCode===specCode&&p.gradeCode===gradeCode&&(p.categoryCode||"")===(categoryCode||""));
    if(dupCombo) return showToast(`This combination already exists as ${dupCombo.sku}.`,"error");
    setData(prev=>({...prev,products:[...prev.products,{id:genId(),sku:skuClean,modelCode,storageCode,colourCode,specCode,gradeCode,categoryCode:categoryCode||"",purchasePrice,salePrice}]}));
    showToast(`Product ${skuClean} created.`); setNewProduct({modelCode:"",storageCode:"",colourCode:"",specCode:"",gradeCode:"",categoryCode:"",sku:"",purchasePrice:"",salePrice:""});
  };
  const deleteProduct=id=>{ if(data.trays.find(t=>t.productId===id)) return showToast("Product is used by a tray — cannot remove.","error"); setData(prev=>({...prev,products:prev.products.filter(p=>p.id!==id)})); showToast("Product removed."); };

  // Catalog helpers
  const mkAdd=(listKey,allList,newVal,setNewVal,label)=>()=>{
    ...
    setData(prev=>({...prev,[listKey]:[...(prev[listKey]||[]),{code,label:lbl}]}));
    setNewVal({code:"",label:""}); showToast(`${label} "${lbl}" added.`);
  };
  const mkDel=(listKey,trayKey,label)=>code=>{ if(data.trays.find(t=>trayKey&&data.products.find(p=>p.id===t.productId)?.[trayKey]===code)) return showToast(`${label} is used by a product — cannot remove.`,"error"); setData(prev=>({...prev,[listKey]:(prev[listKey]||[]).filter(x=>x.code!==code)})); showToast(`${label} removed.`); };
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div style={{background:"#fff",borderRadius:14,padding:22,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
          ...
          {data.users.map(u=>(
            <div key={u.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",borderRadius:10,background:"#f8fafc",marginBottom:8}}>
              <div><div style={{fontWeight:700,fontSize:14}}>{u.username} {u.id===session.user.id&&<span style={{fontSize:11,color:"#94a3b8"}}>(you)</span>}</div><RoleBadge role={u.role}/></div>
              <div style={{display:"flex",gap:6}}>
                ...
                {u.id!==session.user.id&&<button onClick={()=>deleteUser(u.id)} style={{background:"#fef2f2",border:"none",borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:12,fontWeight:600,color:"#dc2626"}}>Remove</button>}
              </div>
            </div>
          ))}
        </div>
        <div style={{background:"#fff",borderRadius:14,padding:22,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
          ...
          {[["Products",data.products.length],["Total Devices",data.devices.length],["In Stock",data.devices.filter(d=>d.status==="In Stock").length],["Sold",data.devices.filter(d=>d.status==="Sold").length],["Trays",data.trays.length],["Shelves",data.shelves.length],["Session Timeout","10 min"],["IMEI Validation","Strict 15-digit"]].map(([k,v])=>(
      ...
      <div style={{background:"#fff",borderRadius:14,padding:22,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
          ...
          <div>
            <div style={{background:"#f8fafc",borderRadius:10,padding:18,marginBottom:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10}}>
                {[{label:"Model *",key:"modelCode",opts:allModelCodes},{label:"Storage *",key:"storageCode",opts:allStorageCodes},{label:"Colour *",key:"colourCode",opts:allColourCodes},{label:"Spec *",key:"specCode",opts:allSpecCodes},{label:"Grade *",key:"gradeCode",opts:allGradeCodes}].map(f=>(
                  <div key={f.key}>
                    <label style={{fontSize:11,fontWeight:600,color:"#64748b",display:"block",marginBottom:3}}>{f.label}</label>
                    <select value={newProduct[f.key]} onChange={e=>setNewProduct(p=>({...p,[f.key]:e.target.value}))} style={selS}>
                      <option value="">— Select —</option>
                      {f.opts.map(o=><option key={o.code} value={o.code}>{o.label} ({o.code})</option>)}
                    </select>
                  </div>
                ...
                <div>
                  <label style={{fontSize:11,fontWeight:600,color:"#64748b",display:"block",marginBottom:3}}>Category <span style={{fontWeight:400}}>(optional)</span></label>
                  <select value={newProduct.categoryCode} onChange={e=>setNewProduct(p=>({...p,categoryCode:e.target.value}))} style={selS}>
                    <option value="">— None —</option>
                    {allCategories.map(c=><option key={c.code} value={c.code}>{c.label} ({c.code})</option>)}
                  </select>
                </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                <div style={{gridColumn:"1/-1"}}>
                  <label style={{fontSize:11,fontWeight:600,color:"#64748b",display:"block",marginBottom:3}}>SKU * <span style={{fontWeight:400,color:"#94a3b8"}}>(unique, you define it)</span></label>
                  <input value={newProduct.sku} onChange={e=>setNewProduct(p=>({...p,sku:e.target.value.toUpperCase()}))} placeholder="e.g. IP14P128BKU-A" style={{...inpS,width:"100%",fontFamily:"monospace",fontSize:15,fontWeight:700}}/>
                </div>
                <div>
                  <label style={{fontSize:11,fontWeight:600,color:"#64748b",display:"block",marginBottom:3}}>Purchase £</label>
                  <input type="number" value={newProduct.purchasePrice} onChange={e=>setNewProduct(p=>({...p,purchasePrice:e.target.value}))} style={{...inpS,width:"100%"}}/>
                </div>
                <div>
                  <label style={{fontSize:11,fontWeight:600,color:"#64748b",display:"block",marginBottom:3}}>Sale £</label>
                  <input type="number" value={newProduct.salePrice} onChange={e=>setNewProduct(p=>({...p,salePrice:e.target.value}))} style={{...inpS,width:"100%"}}/>
                </div>
                <div style={{display:"flex",alignItems:"flex-end"}}>
                  <button onClick={addProduct} style={{background:"#3b82f6",border:"none",borderRadius:8,padding:"9px 24px",cursor:"pointer",fontWeight:700,color:"#fff",fontSize:14,width:"100%"}}>+ Create Product</button>
                </div>
              </div>
            </div>
            <div style={{maxHeight:340,overflowY:"auto"}}>
              {data.products.length===0&&<div style={{textAlign:"center",padding:30,color:"#94a3b8",fontSize:14}}>No products yet.</div>}
              {data.products.map(p=>(
                <div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderRadius:10,background:"#f8fafc",marginBottom:8}}>
                  <div style={{flex:1}}>
                    <div style={{fontFamily:"monospace",fontWeight:800,fontSize:14,color:"#0f172a"}}>{p.sku}</div>
                    <div style={{fontSize:12,color:"#64748b",marginTop:2}}>{productDesc(p,catalog)}</div>
                  </div>
                  <div style={{display:"flex",gap:10,alignItems:"center",marginLeft:12}}>
                    <div style={{fontSize:12,color:"#64748b",textAlign:"right"}}>
                      <div>Cost: {fmtGBP(p.purchasePrice)}</div>
                      <div>Sale: {fmtGBP(p.salePrice)}</div>
                    </div>
        ...
        {catalogTab==="models"&&<CatalogSection title="Model" items={data.customModelCodes||[]} builtIn={MODEL_CODES} newVal={newModel} setNewVal={setNewModel} onAdd={mkAdd("customModelCodes",allModelCodes,newModel,setNewModel,"Model")} onDelete={mkDel("customModelCodes","modelCode","Model")} codePlaceholder="e.g. IP17P" labelPlaceholder="e.g. iPhone 17 Pro" codeHint="IP=iPhone, SGS=Samsung Galaxy S, GP=Google Pixel"/>}
        {catalogTab==="colours"&&<CatalogSection title="Colour" items={data.customColourCodes||[]} builtIn={COLOUR_CODES} newVal={newColour} setNewVal={setNewColour} onAdd={mkAdd("customColourCodes",allColourCodes,newColour,setNewColour,"Colour")} onDelete={mkDel("customColourCodes","colourCode","Colour")} codePlaceholder="e.g. OR" labelPlaceholder="e.g. Orange" codeHint="2-3 letters. e.g. OR=Orange, RG=Rose Gold"/>}
        {catalogTab==="storage"&&<CatalogSection title="Storage" items={data.customStorageCodes||[]} builtIn={STORAGE_CODES} newVal={newStorage} setNewVal={setNewStorage} onAdd={mkAdd("customStorageCodes",allStorageCodes,newStorage,setNewStorage,"Storage")} onDelete={mkDel("customStorageCodes","storageCode","Storage")} codePlaceholder="e.g. 2T" labelPlaceholder="e.g. 2TB" codeHint="Keep consistent with existing format."/>}
        {catalogTab==="specs"&&<CatalogSection title="Spec" items={data.customSpecCodes||[]} builtIn={SPEC_CODES} newVal={newSpec} setNewVal={setNewSpec} onAdd={mkAdd("customSpecCodes",allSpecCodes,newSpec,setNewSpec,"Spec")} onDelete={mkDel("customSpecCodes","specCode","Spec")} codePlaceholder="e.g. E" labelPlaceholder="e.g. European" codeHint="U=UK, G=US/Global, J=Japanese"/>}
        {catalogTab==="grades"&&<CatalogSection title="Grade" items={data.customGradeCodes||[]} builtIn={GRADE_CODES} newVal={newGrade} setNewVal={setNewGrade} onAdd={mkAdd("customGradeCodes",allGradeCodes,newGrade,setNewGrade,"Grade")} onDelete={mkDel("customGradeCodes","gradeCode","Grade")} codePlaceholder="e.g. S" labelPlaceholder="e.g. Grade S (Sealed)" codeHint="A, B, C, R are built-in."/>}
        {catalogTab==="categories"&&<CatalogSection title="Category" items={data.customCategories||[]} builtIn={[]} newVal={newCategory} setNewVal={setNewCategory} onAdd={mkAdd("customCategories",allCategories,newCategory,setNewCategory,"Category")} onDelete={mkDel("customCategories",null,"Category")} codePlaceholder="e.g. MC" labelPlaceholder="e.g. Matching Color" codeHint="Used to differentiate product variants e.g. Matching Color, Bundle, Sealed."/>}
      </div>
      ...
      {changePIN&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"#fff",borderRadius:16,padding:28,width:320,boxShadow:"0 20px 60px rgba(0,0,0,0.25)"}}>
            ...
            <div style={{fontSize:13,color:"#64748b",marginBottom:14}}>User: <b>{data.users.find(u=>u.id===changePIN)?.username}</b></div>
