...
function LoginScreen({onLogin}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0f172a",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#fff",borderRadius:20,padding:40,width:"100%",maxWidth:360,boxShadow:"0 20px 60px rgba(0,0,0,0.4)",boxSizing:"border-box"}}>
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"#f1f5f9",minHeight:"100vh"}}>
      ...
      <div style={{background:"#0f172a",color:"#fff",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
      ...
      <div style={{background:"#1e293b",padding:"0 8px",display:"flex",overflowX:"auto"}}>
        {availTabs.map(t=><button key={t} onClick={()=>setTab(t)} style={{background:"none",border:"none",padding:"12px 14px",cursor:"pointer",fontSize:13,fontWeight:tab===t?700:400,color:tab===t?"#60a5fa":"#94a3b8",borderBottom:tab===t?"3px solid #60a5fa":"3px solid transparent",whiteSpace:"nowrap"}}>{tabLabels[t]}</button>)}
      </div>
...
function ScannerTab({devices,trays,trayStock,trayProduct,imeiMap,showToast,session,catalog,loadAll}) {
  ...
  const phaseLabel={idle:"Scan a Tray SKU to begin",mode_select:`Tray ${product?.sku||""} — Choose mode`,intake:`Intake → ${product?.sku||""}`,intake_confirm:`Confirm Intake — ${product?.sku||""}`,dispatch:`Dispatch → ${product?.sku||""}`,dispatch_confirm:`Confirm Dispatch — ${product?.sku||""}`};
        ...
        <div style={{background:"#0f172a",borderRadius:14,padding:20,marginBottom:14}}>
          ...
          {phase==="mode_select"&&(
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <button onClick={()=>{setPhase("intake");setScanLog([]);setQueue([]);}} style={{background:"#064e3b",border:"2px solid #10b981",borderRadius:12,padding:"18px",cursor:"pointer",color:"#fff",textAlign:"center"}}><div style={{fontSize:28,marginBottom:6}}>📥</div><div style={{fontWeight:800,fontSize:16}}>Intake</div><div style={{fontSize:12,color:"#6ee7b7",marginTop:4}}>Add new stock</div></button>
              <button onClick={()=>{setPhase("dispatch");setScanLog([]);setQueue([]);}} style={{background:"#450a0a",border:"2px solid #ef4444",borderRadius:12,padding:"18px",cursor:"pointer",color:"#fff",textAlign:"center"}}><div style={{fontSize:28,marginBottom:6}}>📤</div><div style={{fontWeight:800,fontSize:16}}>Dispatch</div><div style={{fontSize:12,color:"#fca5a5",marginTop:4}}>Remove stock</div></button>
            </div>
          )}
          <div style={{display:"flex",gap:10,marginTop:phase==="mode_select"?10:0}}>
            <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value.toUpperCase())} onKeyDown={e=>e.key==="Enter"&&handleScan()} placeholder={phase==="idle"?"Scan tray SKU...":phase==="intake"?"Scan IMEI to add...":phase==="dispatch"?"Scan IMEI to dispatch...":""} disabled={inputDisabled} style={{flex:1,padding:"13px 16px",borderRadius:10,border:`2px solid ${modeColor[phase]}`,fontSize:15,fontFamily:"monospace",background:"#1e293b",color:"#f1f5f9",outline:"none",letterSpacing:"0.05em",opacity:inputDisabled?0.4:1,minWidth:0}}/>
          ...
          {(phase==="intake"||phase==="dispatch")&&(
            <div style={{marginTop:12,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
              ...
              <button onClick={()=>{ if(!queue.length){showToast("Nothing scanned.","error");return;} setPhase(phase==="intake"?"intake_confirm":"dispatch_confirm"); }} style={{marginLeft:"auto",background:phase==="intake"?"#10b981":"#ef4444",border:"none",borderRadius:8,padding:"9px 22px",cursor:"pointer",fontWeight:800,color:"#fff",fontSize:14}}>Done →</button>
            </div>
          )}
        ...
        {scanLog.length>0&&(
          <div style={{background:"#fff",borderRadius:12,padding:18,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
            ...
            <div style={{maxHeight:220,overflowY:"auto"}}>
              {scanLog.map((s,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 10px",borderRadius:6,background:s.ok?"#f0fdf4":"#fef2f2",marginBottom:4}}><span>{s.ok?"✅":"❌"}</span><span style={{fontFamily:"monospace",fontSize:13,flex:1}}>{s.imei}</span><span style={{fontSize:12,color:s.ok?"#065f46":"#dc2626",fontWeight:600}}>{s.msg}</span></div>))}
            </div>
          </div>
        )}
        {tray&&(
          <div style={{background:"#fff",borderRadius:12,padding:18,boxShadow:"0 2px 8px rgba(0,0,0,0.07)",marginTop:12}}>
  ...
  return (
    <div>
      <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
        <input value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="New shelf name" style={{padding:"9px 14px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:14,flex:"1 1 200px"}}/>
...
function TraysTab({trays,products,shelves,trayStock,trayDevices,trayProduct,shelfTrays,devices,showToast,catalog,loadAll,session}) {
  ...
  const selStyle={width:"100%",padding:"9px 12px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:14,boxSizing:"border-box"};
    ...
    <div>
            ...
            <div key={tray.id} style={{background:"#fff",borderRadius:12,padding:18,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                ...
                <div style={{display:"flex",gap:5}}>
                  ...
                  {stock>0&&<button onClick={()=>{setClearTray(tray);setClearReason("");}} style={{background:"#fff7ed",border:"none",borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:12,fontWeight:600,color:"#ea580c"}}>🗑</button>}
              ...
              {p&&<div style={{display:"flex",justifyContent:"space-between",marginTop:10,fontSize:12,color:"#64748b"}}><span>Cost: {fmtGBP(p.purchase_price)}</span><span>Sale: {fmtGBP(p.sale_price)}</span><span style={{fontWeight:700,color:"#10b981"}}>Val: {fmtGBP(parseFloat(p.sale_price||0)*stock)}</span></div>}
            </div>
          );
        })}
      </div>
      ...
      {showAdd&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:"#fff",borderRadius:16,padding:28,width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.25)",boxSizing:"border-box"}}>
            ...
            {noMatch&&<div style={{background:"#fef2f2",border:"1px solid #fca5a5",borderRadius:10,padding:"12px 16px",marginBottom:16}}><div style={{fontSize:13,fontWeight:700,color:"#dc2626",marginBottom:2}}>⚠️ No product found</div><div style={{fontSize:12,color:"#64748b"}}>Go to <b>Admin → Product Catalog → Products</b> and create it first.</div></div>}
      ...
      {clearTray&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:"#fff",borderRadius:16,padding:28,width:"100%",maxWidth:420,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",boxSizing:"border-box"}}>
...
function PrintModal({tray,shelves,trayProduct,catalog,onClose}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#fff",borderRadius:16,padding:32,width:"100%",maxWidth:440,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",boxSizing:"border-box"}}>
  ...
  return (
    <div>
      <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
        ...
        <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{padding:"9px 12px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:14}}><option value="All">All Statuses</option>{STATUSES.map(s=><option key={s} value={s}>{s}</option>)}</select>
        <select value={filterTray} onChange={e=>setFilterTray(e.target.value)} style={{padding:"9px 12px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:14}}><option value="All">All Trays</option>{trays.map(t=>{ const p=trayProduct[t.id]; return <option key={t.id} value={t.id}>{p?.sku||t.id}</option>; })}</select>
...
function HistoryTab({history,devices}) {
  ...
  return (
    <div>
      <div style={{marginBottom:14}}><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 IMEI, user, action..." style={{padding:"9px 14px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:14,width:"100%",maxWidth:300,boxSizing:"border-box"}}/></div>
      <div style={{background:"#fff",borderRadius:12,boxShadow:"0 1px 4px rgba(0,0,0,0.07)",overflow:"auto"}}>
...
function AdminTab({users,shelves,products,showToast,session,catalog,skuSet,loadAll,allModelCodes,allColourCodes,allStorageCodes,allSpecCodes,allGradeCodes,allCategories}) {
  ...
  const inpS={padding:"8px 12px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:13,boxSizing:"border-box"};
  const selS={width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:13,boxSizing:"border-box"};
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:16}}>
        ...
        <div style={{background:"#fff",borderRadius:14,padding:22,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
          ...
          {[["Products",products.length],["Shelves",shelves.length],["Session Timeout","10 min"],["Database","Supabase ☁️"],["Sync","✅ Realtime + 8s"]].map(([k,v])=>(
      ...
      <div style={{background:"#fff",borderRadius:14,padding:22,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
        ...
        {catalogTab==="products"&&(
          <div>
            ...
            <div style={{maxHeight:340,overflowY:"auto"}}>
              ...
              {products.map(p=>(
                <div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderRadius:10,background:"#f8fafc",marginBottom:8,flexWrap:"wrap",gap:8}}>
                  <div style={{flex:"1 1 200px"}}><div style={{fontFamily:"monospace",fontWeight:800,fontSize:14,color:"#0f172a"}}>{p.sku}</div><div style={{fontSize:12,color:"#64748b",marginTop:2}}>{productDesc(p,catalog)}</div></div>
        ...
        {catalogTab==="models"&&<CatalogSection title="Model" items={allModelCodes.filter(m=>!MODEL_CODES.find(b=>b.code===m.code))} builtIn={MODEL_CODES} newVal={newModel} setNewVal={setNewModel} onAdd={mkAdd("custom_model_codes",allModelCodes,newModel,setNewModel,"Model")} onDelete={mkDel("custom_model_codes","Model")} codePlaceholder="e.g. IP17P" labelPlaceholder="e.g. iPhone 17 Pro" codeHint="IP=iPhone, SGS=Samsung, GP=Pixel"/>}
        ...
        {catalogTab==="specs"&&<CatalogSection title="Spec" items={allSpecCodes.filter(s=>!SPEC_CODES.find(b=>b.code===s.code))} builtIn={SPEC_CODES} newVal={newSpec} setNewVal={setNewSpec} onAdd={mkAdd("custom_spec_codes",allSpecCodes,newSpec,setNewSpec,"Spec")} onDelete={mkDel("custom_spec_codes","Spec")} codePlaceholder="e.g. E" labelPlaceholder="e.g. European" codeHint="U=UK, G=Global, J=Japan"/>}
        {catalogTab==="grades"&&<CatalogSection title="Grade" items={allGradeCodes.filter(g=>!GRADE_CODES.find(b=>b.code===g.code))} builtIn={GRADE_CODES} newVal={newGrade} setNewVal={setNewGrade} onAdd={mkAdd("custom_grade_codes",allGradeCodes,newGrade,setNewGrade,"Grade")} onDelete={mkDel("custom_grade_codes","Grade")} codePlaceholder="e.g. S" labelPlaceholder="e.g. Sealed" codeHint="A,B,C,R built-in."/>}
        {catalogTab==="categories"&&<CatalogSection title="Category" items={allCategories} builtIn={[]} newVal={newCategory} setNewVal={setNewCategory} onAdd={mkAdd("custom_categories",allCategories,newCategory,setNewCategory,"Category")} onDelete={mkDel("custom_categories","Category")} codePlaceholder="e.g. MC" labelPlaceholder="e.g. Matching Color" codeHint="Differentiate variants."/>}
      </div>

      {showAddUser&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:"#fff",borderRadius:16,padding:28,width:"100%",maxWidth:360,boxShadow:"0 20px 60px rgba(0,0,0,0.25)",boxSizing:"border-box"}}>
      ...
      {changePIN&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:"#fff",borderRadius:16,padding:28,width:"100%",maxWidth:320,boxShadow:"0 20px 60px rgba(0,0,0,0.25)",boxSizing:"border-box"}}>
